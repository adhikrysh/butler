...
- **SOUL.md** is the persona: a short markdown file loaded at the top of every conversation (calm, concise, "modular butler"). It's also the **one home for global agent policy** — e.g. *skills and code are read-only to you (enforced three ways); the agent logs skill-improvement ideas via the `learnings` skill (`learn.py`), a weekly digest surfaces them, and the user promotes the good ones into a skill via git*. That rule lives here, once, not copy-pasted into each skill (it used to be, and drifted).
