...
- **Data layer:** the four Sheet-backed modules (`ppl-index`, `cold-outbounds`, `superforecasting`, `jim`) read/write through `modules/lib/store.py` — a local **SQLite** DB (`state/butler.db`, `BUTLER_DB_PATH`) is the source of truth; the Google Sheet is a best-effort **write-through view** (fast local reads, clean updates). `jim.py resync` rebuilds the Sheet from the DB on drift.
- **Backups:** the box's `backup_configs.sh` stages the profile + a consistent `butler.db` snapshot into `/srv/sambashare/data/butler-backup/`, which the server's restic → Backblaze B2 job captures nightly (along with everything else under that path). See the server runbook.
