...
- **What it does:** the user sends a link or text in chat and asks Butler to read it; Butler
  reads it aloud in **Ronald**'s voice (Cartesia) and delivers it as **Telegram voice
...
- **Pipeline** (`scripts/speak.py`, self-contained via `uv`):
  ...
  chunk ~250 for a fast start) → **Cartesia** (`sonic-2`, Ronald) mp3 → **ffmpeg** opus/ogg
...
- **Config / secrets** (profile `.env`, never committed): `CARTESIA_API_KEY`,
  ...
  (default Ronald `5ee9feff-1265-424a-9d7f-8e4d431a12c7`), `CARTESIA_MODEL` (default `sonic-2`).
- **Errors:** unreadable page → non-zero exit, agent tells the user (and may retry via tinyfish
...
- **Verified live:** Cartesia key + Ronald voice, `sonic-2` mp3, `imageio-ffmpeg` libopus
